...
def pretraiter_etp_annuelle(df_commune):
    resumyear = []
    for idP in df_commune["idPoint"].unique():
        tmp = df_commune[df_commune["idPoint"] == idP].copy()
        tmp["ETP_Q"] = pd.to_numeric(tmp["ETP_Q"], errors="coerce")
        tmp["date"] = pd.to_datetime(tmp["time"], errors="coerce")
        tmp = tmp.dropna(subset=["date", "ETP_Q"])
...
def afficher_etp_annuelle(resumyear, df_commune, nom_commune, an_debut=1959, an_fin=2024, an_reference=True, periode_ref=(1959, 1987)):
    ...
    rtest = test.groupby('year').apply(
        lambda x: pd.Series({
            'ETP_Qmp': x['ETP_Qp'].sum() / x['Ratio_Comm'].sum()
            if x['Ratio_Comm'].sum() != 0 else float('nan')
        })
    ).reset_index()
...
@st.cache_data
def pretraiter_etp_mensuel(df_commune, an_debut, an_fin):
    resummonth = []
    df = df_commune.copy()
...
def calculer_etp_mensuel_commune(df_monthly, df_commune):
    month_abb = list(calendar.month_abbr)[1:]
    tmpcomm = df_commune[['idPoint', 'Ratio_Comm']].drop_duplicates()
    test = pd.merge(df_monthly, tmpcomm, on="idPoint")
    test["ETP_Qp"] = test["ETP_Q"] * test["Ratio_Comm"]

    rcycle = test.groupby("month").apply(
        lambda x: pd.Series({
            "ETP_Qmp": x["ETP_Qp"].sum() / x["Ratio_Comm"].sum()
        })
    ).reset_index()
