...
# --- 2. Prétraitement mensuel PRECIP ---
@st.cache_data
...
def afficher_precip_annuelle(resumyear, df, nom_commune, an_debut=1959, an_fin=2024, an_reference=True, periode_ref=(1959, 1987), resum_ref=None):
    tmpcomm = df[['idPoint', 'Ratio_Comm']].drop_duplicates()
    test = pd.merge(resumyear, tmpcomm, on='idPoint')
    test['Pp'] = test['P'] * test['Ratio_Comm']
    test['year'] = test['year'].astype(int)

    rtest = test.groupby(['year']).apply(lambda df: pd.Series({
    ...
    if an_reference and resum_ref is not None:
        ...
        rtest_ref = test_ref.groupby(['year']).apply(lambda df: pd.Series({
            'Pmp': (df['Pp'].sum() / df['Ratio_Comm'].sum()) if df['Ratio_Comm'].sum() != 0 else float('nan')
        })).reset_index()
...
# --- 4. Affichage mensuel PRECIP ---
def afficher_precip_mensuel(df_month, df_ratio, nom_commune, an_debut, an_fin, df_month_ref=None):
    month_abb = list(calendar.month_abbr)[1:]
    tmpcomm = df_ratio[['idPoint', 'Ratio_Comm']].drop_duplicates()
...
# --- 5. Titre général précipitations ---
