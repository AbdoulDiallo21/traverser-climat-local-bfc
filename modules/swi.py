...
def pretraiter_swi_mensuel(df, an_debut, an_fin):
    ...
    for idP in df["idPoint"].unique():
        ...
        if not tmp.empty:
            monthly_mean = tmp["SWI_Q"].resample("ME").mean()
            df_month = pd.DataFrame({
                "year": monthly_mean.index.year,
                "month": monthly_mean.index.month,
                "SWI_Q": monthly_mean.values,
                "idPoint": idP
            })
...
def afficher_swi_annuel(resumyear, df, nom_commune, an_debut=1959, an_fin=2024, an_reference=True, periode_ref=(1959, 1987), resum_ref=None):
    ...
    rtest = (
        ...
        .apply(lambda df: pd.Series({
            'SWI_Qmp': (df['SWI_Qp'].sum() / df['Ratio_Comm'].sum()) if df['Ratio_Comm'].sum() != 0 else float('nan')
        }))
        .reset_index()
    )
    ...
    fig.update_layout(
        ...
        title=f"Variation interannuelle de SWI<br>({an_debut}–{an_fin})",
        title_x=0.1,
        xaxis_title="Année",
...
def afficher_swi_mensuel(df_month, df_ratio, nom_commune, an_debut, an_fin, df_month_ref=None,rcycle_ref=None):
    ...
    fig.update_layout(
        ...
        paper_bgcolor="#f9f9f9",
        font=dict(family="Arial", size=14),
        title=f"Cycle annuel moyen de SWI<br>({an_debut}–{an_fin})",
        xaxis_title="Mois",
        yaxis_title="SWI moyen",
        title_x=0.1,
        height=500,
...
def afficher_titre_swi(nom_commune):
    ...
    <div style='background-color:#e3f2fd; padding: 16px; border-left: 6px solid #2196F3;
        ...
        <h3 style='text-align:center; color:#1b2b40; margin: 0; font-family: Arial, sans-serif;'>
            🌱 Indice d'humidité du sol (SWI : Soil Wetness Index) – {nom_commune}
        </h3>
    </div>
    """,
    ...
    st.markdown(
        ...
        <div style="background-color: #f5f5f5; padding: 20px; border-left: 6px solid #9e9e9e;
            ...
            <p>
            L’indice normalisé d’humidité du sol (SWI : <em>Soil Wetness Index</em>) mesure la quantité relative d’eau présente dans le sol. 
            Il représente, sur une profondeur d’environ deux mètres, l’état de la réserve en eau du sol par rapport à la réserve utile (eau disponible pour l’alimentation des plantes).
            </p>
            <p>
            Lorsque le SWI est voisin de 1, voire supérieur à 1, le sol est <strong>humide</strong>, tend vers la <strong>saturation</strong>.<br>
            Lorsqu’il tend vers 0, voire passe en dessous de 0, le sol est en état de <strong>stress hydrique</strong>, voire très <strong>sec</strong>.
            </p>
            <p>
